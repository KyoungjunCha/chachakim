...
const SurveyPage = () => {
    ...
    const [page, setPage] = useState(BOARD_NAME["SURVEY"]);
    const navigate = useNavigate();

    useEffect(() => {
        axios.get("http://localhost:4000/surveys")
            .then(response => {
            ...
            .catch(error => {
                console.error("Error fetching the surveys:", error);
            });
    }, []);
    ...
    const createList = (item) => {
        return item.map((listItem, index) => (
            <div
                ...
                onClick={() => navigate(`/surveyDetail/${listItem.survey_Id}`)}
            >
                <div style={{
                    width: "10vw",
                    textAlign: "center",
